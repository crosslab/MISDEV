...
<head>
	...
	<script type="text/javascript">
		...
		function ajaxmodify(){
			...
			postJson("${ctx}/sys/dict/sj","{type:'汉字'}",function(data){
	            alert("成功"+JSON.stringify(data));
	        },e);
		}
	</script>
...
<body>
	...
	<form:form id="searchForm" modelAttribute="dict" action="${ctx}/sys/dict/" method="post" class="breadcrumb form-search">
		...
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
			...
			<form:select id="type" path="type" class="input-medium">
				...
				<form:options items="${typeList}" itemValue="code" itemLabel="name" htmlEscape="false"/>
			</form:select>
		      &nbsp;&nbsp;
		    <label>描述 ：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="dict">
			<tr>
				<td>${dict.value}</td>
				<td><a href="${ctx}/sys/dict/form?id=${dict.id}&d=v">${dict.label}</a></td>
